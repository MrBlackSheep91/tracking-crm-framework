...
// 🧠 INTELLIGENT PAYLOAD ROUTER
async function routePayload(payload: any) {
  console.log('🧠 Analizando payload para routing inteligente...');
  
  const hasSession = payload.session && payload.session.sessionId;
  const hasEvents = payload.events && Array.isArray(payload.events) && payload.events.length > 0;
  const hasLeadData = payload.email || payload.name || payload.phone;
  
  // 🎯 ROUTING LOGIC
  if (hasLeadData && !hasSession) {
    // Es un lead directo (desde formulario)
    console.log('📧 Routing to LEAD-PROCESSOR (direct lead capture)');
    return await sendToMicroWorkflow(MICROWORKFLOW_URLS.LEAD_PROCESSOR, payload, 'LEAD-PROCESSOR');
  } 
  else if (hasSession && hasEvents) {
    // Es tracking de visitante completo
    console.log('👤 Routing to VISITOR-PROCESSOR (visitor tracking)');
    return await sendToMicroWorkflow(MICROWORKFLOW_URLS.VISITOR_PROCESSOR, { body: payload }, 'VISITOR-PROCESSOR');
  }
  else if (hasSession && !hasEvents) {
    // Es solo session data (sin eventos)
    console.log('🔗 Routing to VISITOR-PROCESSOR (session only)');
    return await sendToMicroWorkflow(MICROWORKFLOW_URLS.VISITOR_PROCESSOR, { body: payload }, 'VISITOR-PROCESSOR');
  }
  else {
    // Fallback - enviar a activity logger para registrar
    console.log('📊 Routing to ACTIVITY-LOGGER (fallback)');
    return await sendToMicroWorkflow(MICROWORKFLOW_URLS.ACTIVITY_LOGGER, payload, 'ACTIVITY-LOGGER');
  }
}

async function sendToMicroWorkflow(url: string, payload: any, workflowName: string, retries = 3, delay = 1000) {
  try {
